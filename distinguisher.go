...
// Modifies a key to withstand the attack by IsProbablyNewHopePublicKey.
func CorrectPseudoPublicKey(r []byte) bool {
	if len(r) < PolyBytes {
		return false
	}
	...
	for i, v := range pk.coeffs {
		pk.coeffs[i] = v%(paramQ+1)
	}
	pk.toBytes(r)
